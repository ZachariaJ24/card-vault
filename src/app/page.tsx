...
import { KpiCard, AreaChartCard, SparklineCell, type ChartDataPoint } from "@/components/heroui-pro";
...
// Build chart data from mock price history
  ...
  return (
    <DashboardLayout>
      <div className="p-6 max-w-[1400px] mx-auto">
            ...
            <CardBody className="p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <Button
                  ...
                  size="sm"
                  variant="light"
                  className="text-default-400 text-xs h-7"
                >
                  View All
                </Button>
              </div>
              ...
              <div className="space-y-2 mb-4">
                {TOP_GAINERS.map((c) => (
                  <Link
                    key={c.id}
                    href={`/card/${c.id}`}
                    className="flex items-center justify-between py-1.5 hover:bg-default-50 -mx-2 px-2 rounded-lg transition-colors"
                  >
                    <div className="flex items-center gap-2 min-w-0">
                      <span className="text-sm">{sportEmoji(c.sport)}</span>
              ...
              <div className="space-y-2">
                {TOP_LOSERS.map((c) => (
                  ...
                  >
                    ...
                    <span className="text-xs font-mono font-semibold text-danger shrink-0">
                      {formatChange(c.change_pct)}
                    </span>
                  </Link>
                ))}
          ...
          <CardBody className="p-0">
            <div className="flex items-center justify-between px-4 pt-4 pb-2">
              <div className="flex items-center gap-2">
                ...
                <Chip size="sm" color="success" variant="flat" classNames={{ content: "text-[0.6rem] font-semibold" }}>
                  Live
                </Chip>
              </div>
              <Button as={Link} href="/market" size="sm" variant="light" className="text-default-400 text-xs h-7">
                Full Market
              </Button>
            </div>
            <Table
              ...
              <TableBody>
                {ENRICHED.map((card) => (
                  <TableRow key={card.id} href={`/card/${card.id}`} className="group">
                    ...
                    <TableCell>
                      <Chip size="sm" variant="flat" classNames={{ content: "text-[0.65rem] font-medium" }}>
                        {card.grade}
                      </Chip>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell className="hidden md:table-cell">
                      <span className={`font-mono font-medium text-xs ${card.change_7d >= 0 ? "text-success" : "text-danger"}`}>
                        {formatChange(card.change_7d)}
                      </span>
                    </TableCell>
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold">Featured Cards</h3>
          </div>
            ...
            {ENRICHED.slice(0, 4).map((card) => (
              <Link key={card.id} href={`/card/${card.id}`}>
                <Card className="border border-default-200 bg-content1 hover:border-default-300 transition-colors cursor-pointer">
                  <CardBody className="p-4">
                    <div className="w-full aspect-[3/4] rounded-lg bg-default-100 flex items-center justify-center mb-3 border border-default-200">
                      <span className="text-4xl">{sportEmoji(card.sport)}</span>
                    </div>
                    <p className="text-xs font-medium truncate">{card.player_name}</p>
                    <p className="text-[0.65rem] text-default-400 truncate mb-2">{card.card_set} · {card.grade}</p>
                    <div className="flex items-center justify-between">
                      <span className="font-mono font-semibold text-sm">{formatCurrency(card.base_price)}</span>
                      <Chip
                        size="sm"
                        variant="flat"
                        color={card.change_pct >= 0 ? "success" : "danger"}
