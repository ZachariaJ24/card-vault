 ...
 * Each entry is a specific card + grade combination (separate market).
 */
...
/** Base card definitions (before grade expansion) */
const BASE_CARDS = [
  // --- Hockey ---
...
/** Grades to generate per sport */
const SPORT_GRADES: Record<string, string[]> = {
  Hockey:     ["PSA 10", "PSA 9", "BGS 9.5", "RAW"],
  Basketball: ["PSA 10", "PSA 9", "BGS 9.5", "RAW"],
  Football:   ["PSA 10", "PSA 9", "BGS 9.5", "RAW"],
  Baseball:   ["PSA 10", "PSA 9", "BGS 9.5", "RAW"],
  Soccer:     ["PSA 10", "PSA 9", "BGS 9.5", "RAW"],
  Pokemon:    ["PSA 10", "PSA 9", "CGC 9.5", "RAW"],
  Magic:      ["PSA 10", "BGS 9.5", "NM", "LP"],
};

/**
 * Expand base cards into grade-specific entries.
 * Each base card generates 4 rows (one per grade tier).
 */
export const SEED_CARDS: SeedCard[] = BASE_CARDS.flatMap((card) => {
  const grades = SPORT_GRADES[card.sport] ?? ["PSA 10", "PSA 9", "RAW"];
...
/** Get just the base cards (no grade expansion) for UI display */
export const BASE_CARD_LIST = BASE_CARDS;
